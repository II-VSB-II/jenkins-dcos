...
#CMD ["/bin/sh", "-c" , "envsubst '\\$PORT0 \\$PORT1 \\$JENKINS_CONTEXT' < /var/nginx/nginx.conf.template > /var/nginx/nginx.conf && nginx && java ${JVM_OPTS} -Dhudson.model.DirectoryBrowserSupport.CSP=\"${JENKINS_CSP_OPTS}\" -Dhudson.udp=-1 -Djava.awt.headless=true -Dhudson.DNSMultiCast.disabled=true -Djenkins.install.runSetupWizard=false -Djenkins.model.Jenkins.slaveAgentPort=${PORT2} -Djenkins.model.Jenkins.slaveAgentPortEnforce=true -jar ${JENKINS_FOLDER}/jenkins.war ${JENKINS_OPTS} --httpPort=${PORT1} --webroot=${JENKINS_FOLDER}/war --ajp13Port=-1 --httpListenAddress=127.0.0.1 --ajp13ListenAddress=127.0.0.1 --prefix=${JENKINS_CONTEXT}"]
